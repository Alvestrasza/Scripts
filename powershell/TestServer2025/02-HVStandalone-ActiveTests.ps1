...
<#
.SYNOPSIS
Active functional tests for a Windows Server 2025 standalone Hyper-V host.

.DESCRIPTION
Optionally creates and starts a test VM, restarts a test VM, changes dynamic memory configuration,
enables SR-IOV weight for a VM adapter, and optionally performs export/import based backup and restore tests.

.NOTES
File Name     : 02-HVStandalone-ActiveTests.ps1
Version       : v0.1.0
Created       : 2026-06-25
...
Initialize-TestRun -OutputRoot $OutputRoot -RunName "HVStandalone-ActiveTests"

# VM Operation: Create VM
try {
    if ($CreateTestVm) {
...
# VM Operation: Live configuration
try {
    if ($TestLiveConfiguration) {
        ...
        $memory = Get-VMMemory -VMName $TestVmName
...
# Network: SR-IOV for VM adapter
try {
    if ($EnableSriovForVm) {
        Set-VMNetworkAdapter -VMName $TestVmName -IovWeight 100 -ErrorAction Stop
        $adapter = Get-VMNetworkAdapter -VMName $TestVmName
