...
# -*- coding: utf-8 -*-
#
# File Name     : solana_swap_training_tracker_v0.2.0.py
# Version       : v0.2.0
# Created       : 2026-06-29
# Last Modified : 2026-06-29
# Author        : Alice Endelgard / Nouramon Alvestrasza
# Organization  : Alvestrasza Corporation
# Description   : Snapshot anonymous Solana token holders and evaluate swap training transactions without linking names to wallets.
...
1. Snapshot all current holders of Token X into an anonymous wallet source CSV.
...
def read_wallet_sources(path: Path) -> List[WalletSource]:
    ...
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        if "wallet" not in fieldnames:
            raise ValueError("Wallet source CSV must contain a 'wallet' column.")
        ...
        for index, row in enumerate(reader, start=1):
            ...
            wallet_id = (row.get("wallet_id") or f"W{index:06d}").strip()
            wallets.append(
                WalletSource(
                    wallet_id=wallet_id,
                    wallet=wallet,
                    source_mint=(row.get("source_mint") or "").strip(),
                    source_amount=(row.get("source_amount") or "").strip(),
                    source_rank=(row.get("source_rank") or "").strip(),
                    source_token_account=(row.get("source_token_account") or "").strip(),
                    snapshot_time=(row.get("snapshot_time") or "").strip(),
                )
            )
